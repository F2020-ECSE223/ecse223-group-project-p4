...
public class FlexiBookController {
	 ...
	 * 
	 */
	public static void startAppointment(String customerName, String startTime, String date, Date SystemDate,
			Time SystemTime, FlexiBook flexiBook) throws InvalidInputException {
	 ...
	 * 
	 */
	public static void endAppointment(String customerName, String startTime, String date, Date todaysDate,
			Time currentTime, FlexiBook flexiBook) throws InvalidInputException {
	 ...
	 * 
	 */
	public static Appointment cancelAndBookNewService(String username, String service, String newService,
			List<String> optionalServices, String startTime, String date, Date todaysDate, FlexiBook flexiBook)
		...
		if (todaysDate.before(appointmentDate) && !todaysDate.equals(appointmentDate)) {
			...
			try {
				...
				appointmentReturned = FlexiBookController.makeAppointment(username, newService, optionalServices,
						startTime, date, flexiBook, todaysDate);

			} catch (InvalidInputException e) {
				// booking new appointment fails so restore original appointment
				return FlexiBookController.makeAppointment(username, service, optionalServices, startTime, date,
						flexiBook, todaysDate);
				// throw new InvalidInputException(e.getMessage());
			}
		}
	...
	public static Appointment makeAppointment(String username, String mainServiceName,
		...
		try {
			...
			if (optionalServiceNames != null) {
				 optionalServicesString = String.join(delim, optionalServiceNames);
			}
			...
			// Appointment appoint = new Appointment(customer,thisService, flexiBook);
			if (thisService.getClass().equals(ServiceCombo.class) && optionalServiceNames == null) {
				timeSlot = findTimeSlot(startTime, startDate, "", mainServiceName, flexiBook);
			}
			if (thisService.getClass().equals(ServiceCombo.class) && optionalServiceNames != null) {

				timeSlot = findTimeSlot(startTime, startDate, optionalServicesString, mainServiceName, flexiBook);
			}
			...
			appoint = new Appointment(customer, thisService, timeSlot, flexiBook);
			if (thisService.getClass().equals(ServiceCombo.class)) { // the bookableService is a ServiceCombo
				...
				for (ComboItem coi : combo.getServices()) {
					...
					if(optionalServiceNames !=null) {
					arrayOfStrings = optionalServicesString.split(",");
					
					if (arrayOfStrings.length != 0) {
						for (String element : arrayOfStrings) {
							if (element.equals(coi.getService().getName())) {
								appoint.addChosenItem(coi);
							}
						}
					}
...
//			}
	 ...
	 * @return
	 */
	private static TimeSlot findTimeSlot(String startTimeString, String startDate, String optionalServices,
			String serviceName, FlexiBook flexiBook) {
		Date date = Date.valueOf(startDate);
		Time startTime = Time.valueOf(startTimeString + ":00");
		Time endTime = null;
		int duration = 0;
		BookableService bookableService = findServiceByName(serviceName, flexiBook);

		if (optionalServices.equals(null)) {
			Service service = (Service) bookableService;
			duration = service.getDuration();
		
		
		} else if (optionalServices.equals("")) {
			ServiceCombo servCombo = (ServiceCombo) bookableService;
			for (ComboItem coi : servCombo.getServices()) {
				if (coi.isMandatory()) {
					duration += coi.getService().getDuration();
			...
			String[] options = optionalServices.split(",");
			for (int i = 0; i < options.length; i++) {
				Service serv = findServiceByAName(options[i], flexiBook);
				duration += serv.getDuration();
			}
		}
		String[] splitStartTime = startTimeString.split(":");
		Integer hours = Integer.parseInt(splitStartTime[0]);
		Integer minutes = Integer.parseInt(splitStartTime[1]);
		LocalTime localStartTime = LocalTime.of(hours, minutes);
		LocalTime localEndTime = localStartTime.plusMinutes(duration);
		endTime = Time.valueOf(localEndTime);

		TimeSlot timeS = new TimeSlot(date, startTime, date, endTime, flexiBook);
		return timeS;
	 ...
	 * @throws InvalidInputException
	 */
	public static String updateAppointmentTime(Appointment appoint, String username, String serviceName,
			String newStartTime, String newDate, Time oldStartTime, Date oldDate, Date todaysDate, FlexiBook flexiBook)
			throws InvalidInputException {
		...
		try {
			...
			// ONLY ALLOW TIME SLOT UPDATE IF APPOINTMENT STATE IS BOOKED AND NOT
			// IN-PROGRESS
...
//			for (int i = 0; i < appointmentList.size(); i++) {
//				//Appointment thisAppointment = appointmentList.get(i);
			if (oldDate.equals(appoint.getTimeSlot().getStartDate())
					&& oldStartTime.equals(appoint.getTimeSlot().getStartTime())
					&& serviceName.equals(appoint.getBookableService().getName())) {
				appointment = appoint;
				// break;
			}
			// }
...
/////////////////////////////////////////////////////////LOGIN, LOGOUT AND VIEW APPOINTMENT CALENDAR//////////////////////////////////////////////////////////////////
	...
	public static void LogIn(String username, String password) throws InvalidInputException {
		...
		if (username != User.getWithUsername(username).getUsername()
				|| password != User.getWithUsername(username).getPassword()) {
